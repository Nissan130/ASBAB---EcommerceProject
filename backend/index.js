  ...
  db.query(checkEmailQuery, [email], (err, result) => {
    ...
    bcrypt.hash(password, 10, (err, hash) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Server error during password encryption" });
      }
      ...
      db.query(
        insertQuery,
        [fullname, username, email, mobile, hash],
        (err, result) => {
          if (err) {
...
app.get("/search-suggestions", (req, res) => {
  ...
  db.query(searchQuery, [searchTerm, searchTerm], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error fetching search suggestions" });
    }
    ...
    if (result.length > 0) {
      ...
      db.query(deleteQuery, [userId, productId], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error deleting favorite item" });
        }
        return res
          .status(200)
          .json({ message: "Favorite item removed successfully" });
      });
    } else {
      ...
      db.query(insertQuery, [productId, userId], (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Error adding to favorite list" });
        }
        return res
          .status(201)
          .json({ message: "Item added to favorite list successfully" });
      });
    }
  });
});
...
app.post("/order", async (req, res) => {
  const { userId, shippingAddress, products, totalAmount, totalQuantity } =
    req.body;
      ...
      db.query(
        ...
        (err, result) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Failed to save pending order details." });
          }
  ...
  `;
    ...
    db.query(getOrderQuery, [transaction_id], (err, rows) => {
      if (err || rows.length === 0) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to fetch pending order details." });
      }
      ...
      db.query(
        ...
        (err, result) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ error: "Failed to save order details." });
          }
        ...
        `;
          db.query(resetCartQuery, [user_id], (err, result) => {
            ...
            res.redirect(
              `http://localhost:5173/payment/result?status=success&tranId=${transaction_id}`
            );
          });
        }
      );
  ...
  app.post("/payment/fail/:tranId", async (req, res) => {
    ...
    res.redirect(
      `http://localhost:5173/payment/result?status=fail&tranId=${tran_id}`
    );
  });
});
...
app.post("/add-product", upload.fields([{ name: "main_img" }, { name: "other_images", maxCount: 4 }]), (req, res) => {
    ...
    const other_images_paths = req.files.other_images
      ? req.files.other_images.map((file) => file.path)
      : []; // Paths for other images
...
app.get("/products/:product_id", (req, res) => {
  ...
  console.log("Requested product ID in product detailed info:", product_id);
...
app.post(
  ...
  (req, res) => {
    ...
    console.log("Product id in updated product: ", product_id);
    // Validate input
    // if (
    //   !product_title ||
    //   !product_keyword ||
    ...
    const main_image_path = req.files.main_image[0].path; // Path of the main image
    const other_images_paths = req.files.other_images
      ? req.files.other_images.map((file) => file.path)
      : []; // Paths for other images
    ...
    db.query(
      ...
      [
        ...
        discount,
        main_image_path,
        JSON.stringify(other_images_paths),
        product_id,
      ],
      (err, result) => {
        if (err) {
